...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        {
            visited.Add(name);
            foreach (string n in GetDirectDependents(name)) // foreach dependent n in dependents of name
            {
                if (n.Equals(start)) // if one of name dependests is also name this will cause a circular dependency
                {
                    throw new CircularException(); // so an exception is thrown
                }
                else if (!visited.Contains(n)) // else if the dependetn is not in the visited cells
                {
                    Visit(start, n, visited, changed);  // we call Visist on it to add it
                }
            }
